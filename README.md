...
При первом запуске бот использует **двухфакторную авторизацию** (2FA) и запрашивает через Telegram:
- Номер телефона
- Код из СМС
- Пароль (если требуется)
- Код для быстрого входа (PIN-код)
